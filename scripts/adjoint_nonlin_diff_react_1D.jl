...
function residual!(R,H,B,ELA,β,npow,dx)
    @inbounds @simd for ix = 2:length(R)-1
        ...
        D_l   = ((∇S_l>0.0)*(H[ix  ]^(npow+2)*1e-4 + H[ix  ]^npow) + (∇S_l<0.0)*(H[ix-1]^(npow+2)*1e-4 + H[ix-1]^npow))*abs(∇S_l)^2
        D_r   = ((∇S_r>0.0)*(H[ix+1]^(npow+2)*1e-4 + H[ix+1]^npow) + (∇S_r<0.0)*(H[ix  ]^(npow+2)*1e-4 + H[ix  ]^npow))*abs(∇S_r)^2
...
function timestep!(dτ,H,B,npow,dx)
    @inbounds @simd for ix = 2:length(H)-1
        ...
        D_l   = ((∇S_l>0.0)*(H[ix  ]^(npow+2)*1e-4 + H[ix  ]^npow) + (∇S_l<0.0)*(H[ix-1]^(npow+2)*1e-4 + H[ix-1]^npow))*abs(∇S_l)^2
        D_r   = ((∇S_r>0.0)*(H[ix+1]^(npow+2)*1e-4 + H[ix+1]^npow) + (∇S_r<0.0)*(H[ix  ]^(npow+2)*1e-4 + H[ix  ]^npow))*abs(∇S_r)^2
        dτ[ix] = 0.9*min(0.05, 0.05*dx/(1e-5 + sqrt(0.5*(D_l + D_r))))
    end
    return
end
...
@views function main()
    ...
    # display(plot(xc,aa)); error("breakpoint")
    println("  done.")
    solve!(fwd_problem)
    ...
    for gd_iter = 1:gd_niter
        ...
        p2 = plot(xc,[βv,β_synt]; title="β"     , label=["current" "synthetic"], linewidth=2)
