...
function ForwardProblem(H,B,Ela,β,niter,ncheck,ϵtol,dx,dmp,npow,a1,a2,c,ε)
    R  = similar(H)
    dR = similar(H)
    dτ = similar(H)
    return ForwardProblem(H,R,dR,dτ,B,Ela,β,niter,ncheck,ϵtol,dx,dmp,npow,a1,a2,c,ε)
end

function solve!(problem::ForwardProblem)
    (;H,R,dR,dτ,B,Ela,β,niter,ncheck,ϵtol,dx,dmp,npow,a1,a2,c,ε) = problem
    nx  = length(H)
    cfl = 1.0/2.1*dx^2
    R .= 0; dR .= 0; dτ .= 0
    Err = zeros(nx)
    ...
    while merr >= ϵtol && iter < niter
        residual!(dR,H,B,Ela,β,dτ,c,npow,a1,a2,cfl,ε,dx)
        @. Err = H
        @. R   = R*dmp + dR
        @. H   = max(0.0, H + dτ*R)
        if iter % ncheck == 0
...
function AdjointProblem(H,H_obs,B,Ela,β,niter,ncheck,ϵtol,dx,dmp,npow,a1,a2,c,ε)
    Ψ     = similar(H)
    R     = similar(H)
    dR    = similar(H)
    dτ    = similar(H)
    ...
    tmp2  = similar(H)
    ∂J_∂H = similar(H)
    return AdjointProblem(Ψ,R,dR,dτ,tmp1,tmp2,∂J_∂H,H,H_obs,B,Ela,β,niter,ncheck,ϵtol,dx,dmp,npow,a1,a2,c,ε)
end

function solve!(problem::AdjointProblem)
    (;Ψ,R,dR,dτ,tmp1,tmp2,∂J_∂H,H,H_obs,B,Ela,β,niter,ncheck,ϵtol,dx,dmp,npow,a1,a2,c,ε) = problem
    nx  = length(Ψ)
    cfl = 1.0/2.1*dx^2
    Ψ .= 0; R .= 0; dR .= 0; dτ .= 0
    @. ∂J_∂H = H - H_obs
...
@views function main()
    ...
    Ela_synt     = @. 3000 + 400 * atan(xc/lx)
    Ela_ini      = 3000 .+ 400.0 .* rand(nx)
    Ela          = copy(Ela_ini)
    # β_synt       = @. β0 + 0.015 * atan(xc/lx)
    # β_ini        = 0.0153 .+ 0.007.*rand(nx)
    β            = fill(β0,nx)
    Jn           = zeros(nx) # cost function gradient
    synt_problem = ForwardProblem(H_obs,B,Ela_synt,β,niter,ncheck,ϵtol,dx,dmp    ,npow,a1,a2,c,ε)
    fwd_problem  = ForwardProblem(H    ,B,Ela     ,β,niter,ncheck,ϵtol,dx,dmp    ,npow,a1,a2,c,ε)
    adj_problem  = AdjointProblem(H,H_obs,B,Ela   ,β,niter,ncheck,ϵtol,dx,dmp_adj,npow,a1,a2,c,ε)
    # action
